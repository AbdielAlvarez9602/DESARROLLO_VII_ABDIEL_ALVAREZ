...
if ($operacion === 'crear' && !empty($_GET['nombre'])) {
    ...
    if ($gestor->eliminar($_GET['id'])) {
        ...
        $notificacion = "Error: No se encontró el producto para eliminar.";
    }
    header("Location: index.php?notificacion=" . urlencode($notificacion));
    exit;
...
$categoriaLegible = $_GET['categoria'] ?? '';
if ($categoriaLegible === 'electronico' || $categoriaLegible === 'alimento' || $categoriaLegible === 'ropa') {
    $listaProductos = array_filter($listaProductos, function($item) use ($categoriaLegible) {
        return $item->categoria === $categoriaLegible;
    });
}

if ($operacion === 'ordenar') {
    usort($listaProductos, function($a, $b) use ($campoOrden, $tipoOrden) {
        if (!isset($a->$campoOrden) || !isset($b->$campoOrden)) return 0;
...
<body>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" name="nombre" 
                                   value="<?php echo htmlspecialchars($itemParaEditar->nombre ?? ''); ?>" required>
                        </div>
                        
                        <div class="col-md-6">
                            <label class="form-label">Descripción</label>
                            <input type="text" class="form-control" name="descripcion" 
                                   value="<?php echo htmlspecialchars($itemParaEditar->descripcion ?? ''); ?>" required>
                        </div>
                        
                        <div class="col-md-4">
                            <label class="form-label">Stock Disponible</label>
                            <input type="number" class="form-control" name="stock" min="0"
                                   value="<?php echo htmlspecialchars($itemParaEditar->stock ?? ''); ?>" required>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select" name="categoria" id="selectorCategoria" required>
                                ...
                                <option value="electronico" <?php echo (isset($itemParaEditar->categoria) && $itemParaEditar->categoria == 'electronico') ? 'selected' : ''; ?>>Electrónico</option>
                                <option value="alimento" <?php echo (isset($itemParaEditar->categoria) && $itemParaEditar->categoria == 'alimento') ? 'selected' : ''; ?>>Alimento</option>
                                <option value="ropa" <?php echo (isset($itemParaEditar->categoria) && $itemParaEditar->categoria == 'ropa') ? 'selected' : ''; ?>>Ropa</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select" name="estado" required>
                                <option value="disponible" <?php echo (isset($itemParaEditar->estado) && $itemParaEditar->estado == 'disponible') ? 'selected' : ''; ?>>Disponible</option>
                                <option value="agotado" <?php echo (isset($itemParaEditar->estado) && $itemParaEditar->estado == 'agotado') ? 'selected' : ''; ?>>Agotado</option>
                                <option value="por_recibir" <?php echo (isset($itemParaEditar->estado) && $itemParaEditar->estado == 'por_recibir') ? 'selected' : ''; ?>>Por Recibir</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4" id="contenedorCampoEspecifico" style="display:none;">
                            ...
                            <div id="campoElectronico" style="display:none;">
                                ...
                                <input type="number" class="form-control" name="garantiaMeses" min="0" 
                                       value="<?php echo htmlspecialchars($itemParaEditar->garantiaMeses ?? ''); ?>">
                            </div>
                            ...
                            <div id="campoAlimento" style="display:none;">
                                ...
                                <input type="date" class="form-control" name="fechaVencimiento"
                                       value="<?php echo htmlspecialchars($itemParaEditar->fechaVencimiento ?? ''); ?>">
                            </div>
    ...
    function actualizarCamposEspecificos() {
        // Ocultar todos los contenedores y sus inputs
        contenedor.style.display = 'none';
        campoElec.style.display = 'none';
        campoAlim.style.display = 'none';
        campoRop.style.display = 'none';
        
        // Deshabilitar inputs para que no se envíen si no son de la categoría seleccionada
        campoElec.querySelector('input').disabled = true;
        campoAlim.querySelector('input').disabled = true;
        campoRop.querySelector('select').disabled = true;
        ...
        if (valor === 'electronico') {
            ...
            campoElec.querySelector('input').disabled = false;
        } else if (valor === 'alimento') {
            contenedor.style.display = 'block';
            campoAlim.style.display = 'block';
            campoAlim.querySelector('input').disabled = false;
        } else if (valor === 'ropa') {
            contenedor.style.display = 'block';
            campoRop.style.display = 'block';
            campoRop.querySelector('select').disabled = false;
        }
    }
