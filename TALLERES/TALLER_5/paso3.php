...
function estudiantesTutoría($estudiantes) {
    ...
    foreach ($estudiantes as $estudiante) {
        if ($estudiante['promedio'] < 75) {
            $necesitan_tutoría[] = $estudiante['nombre'];
        }
    }
    return $necesitan_tutoría;
}
...
$tutoría = estudiantesTutoría($estudiantes);
if (!empty($tutoría)) {
    echo implode(", ", $tutoría) . "\n";
} else {
...
if (!empty($honor)) {
    ...
    echo "No hay estudiantes en el cuadro de honor.\n";
}
