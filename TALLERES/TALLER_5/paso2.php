...
function encontrarProductoMasValioso($inv) {
    $producto_mas_valioso = null;
    $max_valor = 0;

    foreach ($inv as $producto => $info) {
        $valor_actual = $info['cantidad'] * $info['precio'];
        if ($valor_actual > $max_valor) {
            $max_valor = $valor_actual;
            $producto_mas_valioso = $producto;
        }
    }
    return $producto_mas_valioso;
}

$producto_mas_valioso = encontrarProductoMasValioso($inventario);
echo "\nEl producto con el mayor valor total en inventario es: " . $producto_mas_valioso . "\n";
