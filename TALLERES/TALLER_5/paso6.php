...
function analizarCrecimientoVentas($ventas) {
    $reporteCrecimiento = [];
    $maxCrecimiento = -INF; 
    $productoMaxCrecimiento = ['region' => 'N/A', 'producto' => 'N/A'];

    echo "\n--- Análisis de Crecimiento de Ventas (Primer a Último Mes) ---\n";
    ...
    foreach ($ventas as $region => $productos) {
        ...
        foreach ($productos as $producto => $ventasProducto) {
            ...
            if ($primerMes > 0) {
                ...
                $crecimiento = ($ultimoMes > 0) ? INF : 0; 
            }
            
            $reporteCrecimiento[$region][$producto] = $crecimiento;

            $crecimientoFormateado = ($crecimiento === INF) ? 'Crecimiento Infinito' : number_format($crecimiento, 2) . '%';
            echo "  $producto: $crecimientoFormateado\n";

            if ($crecimiento > $maxCrecimiento) {
                $maxCrecimiento = $crecimiento;
                $productoMaxCrecimiento = ['region' => $region, 'producto' => $producto, 'crecimiento' => $crecimiento];
            }
        }
    }

    echo "\n--------------------------------------------------------------\n";
    echo "Mayor crecimiento identificado:\n";
    
    if ($maxCrecimiento === INF) {
        echo "El mayor crecimiento es: Crecimiento Infinito\n";
        echo "  Producto/Región: {$productoMaxCrecimiento['producto']} en {$productoMaxCrecimiento['region']}\n";
    } elseif ($maxCrecimiento > -INF) {
        echo "El mayor crecimiento es: " . number_format($productoMaxCrecimiento['crecimiento'], 2) . "%\n";
        echo "  Producto/Región: {$productoMaxCrecimiento['producto']} en {$productoMaxCrecimiento['region']}\n";
    } else {
         echo "No se pudo calcular el crecimiento.\n";
    }
}
